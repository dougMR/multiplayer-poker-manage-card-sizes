...
const resizeCards = () => {
    ...
    const getNumOppRows = () => {
        ...
        oppSeats.forEach((oppSeat) => {
            ...
            // }
            if (
                getComputedStyle(oppSeat.querySelector(".player")).getPropertyValue("display") !== "none" &&
    ...
    const clampWidth = (widthInVw, isClient) => {
        ...
        const maxHeight = isClient ? 8 : 8 / numOppRows;
        const maxWidth = isClient ? 12 : 12 / numOppCols;
        ...
        console.log("--->",`Math.min(${maxWidth},${widthInVw})`, Math.min(maxWidth, widthInVw));
        const minWidthPx = 40;
